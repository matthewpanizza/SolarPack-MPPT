...
USBEndpoint0DataPut(unsigned char *pucData, unsigned long ulSize)
{
    ...
    if((unsigned long)pucData > 0x3D8000 && (unsigned long)pucData < 0x3F8000)
    {
        ...
        {
            HWREGB(USB0_BASE + USB_O_FIFO0 + (USB_EP_0 >> 2)) = __byte((int*)pucData,1);
            HWREGB(USB0_BASE + USB_O_FIFO0 + (USB_EP_0 >> 2)) = __byte((int*)pucData,0);
            pucData++;
        }
    }
    else
...
USBDeviceEnumHandler(void)
{
    ...
    //
    ulEPStatus |= ((unsigned long)(HWREGH(USB0_BASE + EP_OFFSET(USB_EP_0) + USB_O_RXCSRL1)) <<
...
USBDSetAddress(tUSBRequest *pUSBRequest)
{
    ...
    // phase is complete.
    //
    g_sUSBDeviceState.ulDevAddress = readShort(&(pUSBRequest->wValue)) | DEV_ADDR_PENDING;
...
USBDGetDescriptor(tUSBRequest *pUSBRequest)
{
    ...
    if(ulStall == 0)
    {
        ...
        if(g_sUSBDeviceState.pEP0Data)
        {
            ...
            //
            if(g_sUSBDeviceState.ulEP0DataRemain > readShort(&(pUSBRequest->wLength)))
            {
                g_sUSBDeviceState.ulEP0DataRemain = readShort(&(pUSBRequest->wLength));
            }
...
{
    if(( 0 == readShort(&(pUSBRequest->wIndex))) && ( 0 == readShort(&(pUSBRequest->wValue))))
    {
        //
...
USBDEP0StateTx(void)
{
    ...
    if((unsigned long)g_sUSBDeviceState.pEP0Data > 0x3D8000 && (unsigned long)g_sUSBDeviceState.pEP0Data < 0x3F8000)
