                 ...
                 bool bVerify, HWND hwndNotify)
{
    ...
    if(eRetcode != DFU_OK)
    {
	  ...
	  while( ((ucTemp = *(unsigned char *)pcBinaryImage++) != EOF) && (ucFailed == 0) ) {
		  ...
		  /* Parse hex record into can frames and transmit appropriately */
			...
			//download command
			  ...
			  if(tCurrentHex.address != ulOldAddress && !ucFirst)
			  {
				...
				// Now we really are ready to download the image!
				//
				eRetcode = DFUDownloadBlock(pState, pucBlockBuffer, pulBBIndex, hwndNotify);
				...
				pulBBIndex = 0;
  				...
  				memcpy(pucBlockBuffer + pulBBIndex, tCurrentHex.buf,  tCurrentHex.word_count * 2);
