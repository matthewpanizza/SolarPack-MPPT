...
f2806x_USBVBusIntHandler(void)
{
    ...
    switch(GpioDataRegs.GPBDAT.bit.GPIO39)
    {
...
//Only setup interrupts if requested
    ...
    //Setup Interrupt
    //Diable PIE temporarily and remember previous state
    intStatus = PieCtrlRegs.PIECTRL.bit.ENPIE;
    ...
    //Re-enable PIE if originally enabled
    PieCtrlRegs.PIECTRL.bit.ENPIE = intStatus;
#endif
}
