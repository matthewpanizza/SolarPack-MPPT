...
tDFUQueryTIProtocol g_sDFUProtocol =
{
    ...
    {TI_DFU_PROTOCOL_VERSION_C2000_1 & 0xFF, (TI_DFU_PROTOCOL_VERSION_C2000_1 & 0xFF00) >> 8}
};
...
SendUploadData(unsigned short usLength, tBoolean bAppendHeader)
{
   ...
   if(bAppendHeader)
   {
       ...
       psHdr->ucReserved = 0;
       writeShort(&(psHdr->usStartAddr), ((unsigned long)(g_sNextUpload.pcStart) / 1024));
       ...
       if(bInFlash)
       {
           for(ulLoop = (usToSend - sizeof(tDFUDownloadProgHeader)) / 2; ulLoop;
               ulLoop--)
           {
       ...
       if(bInFlash)
       {
               g_sNextUpload.pcStart += (usToSend - sizeof(tDFUDownloadProgHeader)) / 2;
               g_sNextUpload.ulLength -= (usToSend - sizeof(tDFUDownloadProgHeader)) / 2;
       }
       else
       {
...
void HandleRequestIdle(tUSBRequest *pUSBRequest)
{
    switch(pUSBRequest->bRequest)
    {
        ...
        case USBD_DFU_REQUEST_DNLOAD:
        {
            ...
            {
                USBBLRequestDataEP0( g_pcDFUBuffer, readShort(&(pUSBRequest->wLength)));
            }
            else
            {
                USBBLStallEP0();
...
{        
    ...
    if((ulStart >= readLong(&(g_sDFUDeviceInfo.ulAppStartAddr))) && (ulLength <=
       (readLong(&(g_sDFUDeviceInfo.ulFlashTop)) - readLong(&(g_sDFUDeviceInfo.ulAppStartAddr)))) &&
...
FlashAPIIndex(unsigned long ulAddress, unsigned long ulSize)
{
    ...
    for(ulIdx = FLASH_START_ADDR ; ulIdx <= FLASH_END_ADDR ; ulIdx += FLASH_PAGE_SIZE)
        {
        if((ulIdx >= (unsigned long)ulAddress) && (ulIdx <= ((unsigned long)ulAddress + ulSize)))
            ulTemp = (ulTemp << 1) | 1;
...
ProcessDFUDnloadCommand(tDFUDownloadHeader *pcCmd, unsigned long ulSize)
{
    ...
    switch(pcCmd->ucCommand)
    {
        ...
        case DFU_CMD_READ:
        {
            ...
            //
            if(FlashRangeCheck((unsigned long)readShort(&(psHdr->usStartAddr)) * 1024, readLong(&(psHdr->ulLength))))
            {
                //
        ...
        case DFU_CMD_CHECK:
        {
            ...
            if(FlashRangeCheck((unsigned long)readShort(&(psHdr->usStartAddr)) * 1024, readLong(&(psHdr->ulLength))))
            {
                ...
                //
                for(ulLoop = 0; (ulLoop < readLong(&(psHdr->ulLength))) && ((unsigned long) pusCheck < 0x3F7F80); ulLoop++)
                {
                    if(*pusCheck != 0xFFFF)
                    {
...
void CompactBuffer(unsigned char *pucBuffer, unsigned long ulBufferSize)
{
    ...
    {
            *pucDestWord++ = ((*pucCurrentByte & 0xFF) << 8) | (*(pucCurrentByte + 1) & 0xFF);
            pucCurrentByte += 2;
    }
...
void CopyBLHtoG(void)
{
    ...
    MemCopy((Uint16 *)&BLTableStart, (Uint16 *)&BLTableEnd, (Uint16 *)g_pcCopyBuffer);
    ...
    // Copy Init Section
    MemCopy((Uint16 *)&InitLoadStart, (Uint16 *)&InitLoadEnd, (Uint16 *)g_pcCopyBuffer);
    ...
    // Copy BL Kernel
    MemCopy((Uint16 *)&RamfuncsLoadStart, (Uint16 *)&RamfuncsLoadEnd, (Uint16 *)g_pcCopyBuffer);
    ...
    // Copy Failsafe Entry
    MemCopy((Uint16 *)&FailsafeLoadStart, (Uint16 *)&FailsafeLoadEnd, (Uint16 *)g_pcCopyBuffer);
...
void CopyBLGtoH(void)
{
    ...
    //Copy Bootloader entry table
    FailSafeMemCopy((Uint16 *)&BLTableStart+0x4000, (Uint16 *)&BLTableEnd+0x4000, (Uint16 *)g_pcCopyBuffer);
    ...
    // Copy Init Section
    FailSafeMemCopy((Uint16 *)&InitLoadStart+0x4000, (Uint16 *)&InitLoadEnd+0x4000, (Uint16 *)g_pcCopyBuffer);
    ...
    // Copy BL Kernel
    FailSafeMemCopy((Uint16 *)&RamfuncsLoadStart+0x4000, (Uint16 *)&RamfuncsLoadEnd+0x4000, (Uint16 *)g_pcCopyBuffer);
    ...
    // Copy Failsafe Entry
    FailSafeMemCopy((Uint16 *)&FailsafeRunStart, (Uint16 *)&FailsafeRunStart+(unsigned long)&FailsafeLoadEnd - (unsigned long)&FailsafeLoadStart, (Uint16 *)g_pcCopyBuffer);
...
{                                              
    ...
    MemCopy((Uint16 *)&code_start_f, (Uint16 *)(&code_start_f + 1), (Uint16 *)g_pcCopyBuffer); 
...
{
    // Copy Failsafe Codestart                                             
    MemCopy((Uint16 *)&code_start_n, (Uint16 *)(&code_start_n + 1), (Uint16 *)g_pcCopyBuffer); 
    ...
    while(1)
    {
        ...
        if(HWREGBITR(&g_ulCommandFlags, CMD_FLAG_WRITE))
        {
            ...
            if((ulStart < 0x3F7FF6) && ((ulStart + usUsed) > 0x3F7FF6)){
				...
				//
				// Write the new block of data to the flash before the begin location
				//
				if(Flash2806x_Program((Uint16 *)ulStart, (Uint16 *) g_pcDFUWrite, (0x3F7FF6 - ulStart), &g_FlashStatus))
				{
					...
					//
					// Write the new block of data to the flash after the begin location (CSM)
					//
					if(Flash2806x_Program((Uint16 *)ulStart + (0x3F7FF6 - ulStart) + 0x02, (Uint16 *) ((unsigned long)g_pcDFUWrite + (0x3F7FF6 - ulStart) + 0x02), (usUsed - ((0x3F7FF6 - ulStart) + 0x02)), &g_FlashStatus))
					{
						//Programming Failed for some reason, enter error state
						g_eDFUState = STATE_ERROR;
						g_eDFUStatus = STATUS_ERR_PROG;
	            ...
	            //
	            if(Flash2806x_Program((Uint16 *)ulStart, (Uint16 *) g_pcDFUWrite, usUsed, &g_FlashStatus))
	            {
	                //Programming Failed for some reason, enter error state
	                g_eDFUState = STATE_ERROR;
	                g_eDFUStatus = STATUS_ERR_PROG;
...
void AppCheck(void)
{
    ...
    if(((unsigned long)pAppEntry != 0xFFFFFFFF) && ((unsigned long)pAppEntry    != 0x00000000))
        ((void (*)(void))pAppEntry)();
}
...
void FailsafeEntry(void)
{
    ...
    // Copy BL to RAM
    FailSafeMemCopy(&RamfuncsLoadStart + 0x4000, &RamfuncsLoadEnd + 0x4000, &RamfuncsRunStart);
